...
with cream-component filtering — see `project_summary.md` §5. The wordmark
itself regenerated byte-identical, which confirms the script still reproduces
it exactly. **If you touch that script, re-run it and check `git status`: only
the file you meant to change should move.**
